...
#pragma unroll
 ...
 public:
  ...
  // Constructors
  GroupRadixSortor(const GroupT &g_, const SubGroupT &sg_,
  ...
  void Sort(KeyT (&keys)[KEYS_PER_WORK_ITEM],
            KeyT *out_keys,
            int num_output = NUM_KEYS,
            int begin_bit = 0,
            int end_bit = sizeof(KeyT) * 8) {
    InternalSortImpl<false, false>(keys, keys, out_keys, out_keys,
  ...
  void SortDescending(KeyT (&keys)[KEYS_PER_WORK_ITEM],
                      ...
                      int begin_bit = 0,
                      int end_bit = sizeof(KeyT) * 8) {
    InternalSortImpl<true, false>(keys, keys, out_keys, out_keys,
                                  num_output, begin_bit, end_bit);
  }
  ...
  void SortDescending(KeyT (&keys)[KEYS_PER_WORK_ITEM],
                      ...
                      int begin_bit = 0,
                      int end_bit = sizeof(KeyT) * 8) {
    InternalSortImpl<true, true>(keys, values, out_keys, out_values,
                                 num_output, begin_bit, end_bit);
  }
};
