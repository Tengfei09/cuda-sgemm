...
__global__ void myHGEMMAlignedV1(
    ...
    const int M, const int N, const int K) {
        ...
        int tx = threadIdx.x;
        int wid = tx >> 5;  // warp id

        const int padding_a = 8;
        const int padding_b = 8;
        __shared__ half tile_a[CTAShape::kM][CTAShape::kK + padding_a];
        __shared__ half tile_b[CTAShape::kK][CTAShape::kN + padding_b];

        wmma::fragment<wmma::matrix_a, 16, 16, 16, half, wmma::row_major> frag_a[2][4];
        wmma::fragment<wmma::matrix_b, 16, 16, 16, half, wmma::row_major> frag_b[2][4];
        wmma::fragment<wmma::accumulator, 16, 16, 16, half> frag_c[4][4];
        constexpr int load_a_smem_m = (tid / 4) * 2;
        constexpr int load_a_smem_k = (tid % 4) * 8;
        constexpr int load_b_smem_k = (tid / 32) * 4;
        constexpr int load_b_smem_n = (tid % 32) * 8;
        ...
        for(int bk = 0; bk < (K + CTAShape::kK - 1) / CTAShape::kK; bk++){
            FLOAT4(tile_a[load_a_smem_m    ][load_a_smem_k]) = FLOAT4(A[load_a_gmem_addr]);
            FLOAT4(tile_a[load_a_smem_m + 1][load_a_smem_k]) = FLOAT4(A[load_a_gmem_addr + K]);
            FLOAT4(tile_b[load_b_smem_k    ][load_b_smem_n]) = FLOAT4(B[load_b_gmem_addr]);
            FLOAT4(tile_b[load_b_smem_k + 1][load_b_smem_n]) = FLOAT4(B[load_b_gmem_addr + N]);
            FLOAT4(tile_b[load_b_smem_k + 2][load_b_smem_n]) = FLOAT4(B[load_b_gmem_addr + 2 * N]);
            FLOAT4(tile_b[load_b_smem_k + 3][load_b_smem_n]) = FLOAT4(B[load_b_gmem_addr + 3 * N]);
            __syncthreads();

            load_a_gmem_addr += CTAShape::kK;
            load_b_gmem_addr += CTAShape::kK * N;

            wmma::load_matrix_sync(frag_a[0][0], &tile_a[comp_c_frag_m * 64][0], BK + padding_a);
            wmma::load_matrix_sync(frag_a[0][1], &tile_a[comp_c_frag_m * 64 + 16][0], BK + padding_a);
            wmma::load_matrix_sync(frag_a[0][2], &tile_a[comp_c_frag_m * 64 + 32][0], BK + padding_a);
            wmma::load_matrix_sync(frag_a[0][3], &tile_a[comp_c_frag_m * 64 + 48][0], BK + padding_a);
            wmma::load_matrix_sync(frag_a[1][0], &tile_a[comp_c_frag_m * 64][16], BK + padding_a);
            wmma::load_matrix_sync(frag_a[1][1], &tile_a[comp_c_frag_m * 64 + 16][16], BK + padding_a);
            wmma::load_matrix_sync(frag_a[1][2], &tile_a[comp_c_frag_m * 64 + 32][16], BK + padding_a);
            wmma::load_matrix_sync(frag_a[1][3], &tile_a[comp_c_frag_m * 64 + 48][16], BK + padding_a);

            wmma::load_matrix_sync(frag_b[0][0], &tile_b[0][comp_c_frag_n * 64], BK + padding_b);
            wmma::load_matrix_sync(frag_b[0][1], &tile_b[0][comp_c_frag_n * 64 + 16], BK + padding_b);
            wmma::load_matrix_sync(frag_b[0][2], &tile_b[0][comp_c_frag_n * 64 + 32], BK + padding_b);
            wmma::load_matrix_sync(frag_b[0][3], &tile_b[0][comp_c_frag_n * 64 + 48], BK + padding_b);
            wmma::load_matrix_sync(frag_b[1][0], &tile_b[16][comp_c_frag_n * 64], BK + padding_b);
            wmma::load_matrix_sync(frag_b[1][1], &tile_b[16][comp_c_frag_n * 64 + 16], BK + padding_b);
            wmma::load_matrix_sync(frag_b[1][2], &tile_b[16][comp_c_frag_n * 64 + 32], BK + padding_b);
            wmma::load_matrix_sync(frag_b[1][3], &tile_b[16][comp_c_frag_n * 64 + 48], BK + padding_b);
