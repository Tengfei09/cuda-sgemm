...
class GroupRadixSelector {
 private:
  ...
  // Rank type
  using GroupRadixRank = GroupRadixRank<GROUP_SIZE, SUB_GROUP_SIZE,
                                        GroupT, RADIX_BITS>;
  ...
  union _LocalStorage {
    typename GroupRadixRank::LocalStorage rank_storage;
...
#pragma unroll
  ...
  void WriteOutKeys(UnsignedT (&ukeys)[KEYS_PER_WORK_ITEM],
                    UnsignedT *output,
                    uint16_t (&ranks)[KEYS_PER_WORK_ITEM],
                    uint16_t offset_select,
                    int num_selected) {
#pragma unroll
  ...
  // Output values
  void WriteOutValues(ValueT (&values)[KEYS_PER_WORK_ITEM],
                      ValueT *output,
                      uint16_t (&ranks)[KEYS_PER_WORK_ITEM],
                      uint16_t offset_select,
                      int num_selected) {
#pragma unroll
  ...
  void InternalSelectImpl(KeyT (&keys)[KEYS_PER_WORK_ITEM],
                          ValueT (&values)[KEYS_PER_WORK_ITEM],
                          KeyT *out_keys,
                          ValueT *out_values,
                          int num_topk,
                          int begin_bit,
    ...
    GroupRadixRank radix_rank(g, sg, local_id, local_mem.rank_storage);
    ...
    while (true) {
      ...
      if (offset_select > 0) {
        ...
        if (IS_PAIR_SELECT)
          WriteOutValues(values, out_values, ranks, offset_select, num_selected);
      }
 ...
 public:
  ...
  void SelectTopK(KeyT (&keys)[KEYS_PER_WORK_ITEM],
                  KeyT *out_keys,
                  int num_topk,
                  int begin_bit = sizeof(KeyT) * 8,
                  int end_bit = 0) {
    InternalSelectImpl<false>(keys, keys, out_keys, out_keys,
                              num_topk, begin_bit, end_bit);
  }
  ...
  void SelectTopK(KeyT (&keys)[KEYS_PER_WORK_ITEM],
                  ...
                  int begin_bit = sizeof(KeyT) * 8,
                  int end_bit = 0) {
    InternalSelectImpl<true>(keys, values, out_keys, out_values,
                             num_topk, begin_bit, end_bit);
  }
};
...
class GroupRadixPerBitSelector {
 private:
  ...
  // GroupScan type
  using GroupScan = GroupScan<int, GROUP_SIZE, SUB_GROUP_SIZE, GroupT>;
...
#pragma unroll
    ...
    GroupScan(g, sg, local_id, local_mem.group_scan).ExclusiveSum(
        item_valid, item_offset, total_valid);
...
#pragma unroll
    ...
    GroupScan group_scan(g, sg, local_id, local_mem.group_scan);
...
#pragma unroll
      for (int i = 0; i < KEYS_PER_WORK_ITEM; ++i) {
        uint32_t desired = ((ukeys[i] & bit_mask) ^ desired_mask) == 0;
      ...
      group_scan.ExclusiveSum(item_count, num_selected,
                              item_offset, group_count);

      // update indexes of selected elements
      if (group_count > num_selected && (group_count <= num_topk ||
                                         begin_bit == end_bit)) {
#pragma unroll
        for (int i = 0; i < KEYS_PER_WORK_ITEM; ++i) {
 ...
 public:
  ...
  void SelectTopK(KeyT (&keys)[KEYS_PER_WORK_ITEM],
                  ...
                  int begin_bit = sizeof(KeyT) * 8,
                  int end_bit = 0) {
    if (CheckKeys(keys, values, out_keys, out_values,
                  num_topk, threshold, num_selected)) {
      InternalSelectImpl<true>(keys, values, out_keys, out_values,
                               num_topk, begin_bit - 1, end_bit);
    }
  }
